...
本教程源于 2025 年的学习与探索记录。那些反复斟酌的思考、在案例中一次次暴露出来的问题，以及每次追问"为什么"之后才慢慢清晰的答案，被重新整理成了这组连贯的篇章。

学 React，最大的诱惑是跳过基础直接上手组件库。页面快速成形，看起来很有效率。但框架在帮你决定的事情太多，一旦遇到它没有预料到的情况，你会发现自己无处着力。本教程选择另一条路：从最简单的 CDN 引入开始，到手动搭建构建环境，再到逐一理解每一个 Hook 的设计意图。框架不是魔法，它只是把你本来就该懂的事情，用更整齐的语法包了起来。

教程以四个案例贯穿始终：**聊天机器人**引出 `useState`、`useRef`、`useEffect` 的核心用法；**搜索关键字联想**展示 `useEffect` cleanup 在竞态条件下不可或缺的作用；**购物车**演示了状态管理从零开始的完整演进——从单组件 `useState`，到 `useContext`，到 `useReducer`，最终到 `Redux`；**图书检索台**则以 200 本书的规模，把 React 默认渲染行为的代价暴露出来，再用 `memo`、`useMemo`、`useCallback` 三件套逐一修复。
...
完成这十章，你已经能独立构建一个有状态管理、有性能意识的 React 应用。但 React 的世界不止于此。
